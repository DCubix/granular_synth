...
int smol_audio_playback_init(int sample_rate, int num_channels, int device) {
	...
	if(render_wave_format->wFormatTag == WAVE_FORMAT_EXTENSIBLE) {
			...
			USHORT gl_id = EXTRACT_WAVEFORMATEX_ID(&ext->SubFormat);
...
#define smol_audio_playback_init_default(sample_rate, num_channels) smol_audio_playback_init(sample_rate, num_channels, -1)
	...
	while(WaitForSingleObject(smol__audio_context->rendering_process_ended, 0) != WAIT_OBJECT_0) {
		...
		if(buffer_data) {
			...
			if(smol__audio_context->render_wave_format.wFormatTag == WAVE_FORMAT_EXTENSIBLE) {
				USHORT gl_id = EXTRACT_WAVEFORMATEX_ID(&smol__audio_context->render_wave_format_ext.SubFormat);
				
				if(gl_id == WAVE_FORMAT_IEEE_FLOAT) {
