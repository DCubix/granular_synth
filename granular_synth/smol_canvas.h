...
#define SMOLC_DARKER_YELLOW		SMOL_RGB(64, 64,     0)
#define SMOLC_DARKEST_YELLOW	SMOL_RGB(31, 31,     0)

#define SMOLC_CYAN				SMOL_RGB(  0, 255, 255)
#define SMOLC_DARK_CYAN			SMOL_RGB(  0, 127, 127)
#define SMOLC_DARKER__CYAN		SMOL_RGB(  0,  63,  63)
#define SMOLC_DARKEST_CYAN		SMOL_RGB(  0,  32,  32)

#define SMOLC_MAGENTA			SMOL_RGB(255,   0, 255)
#define SMOLC_DARK_MAGENTA		SMOL_RGB(127,   0, 127)
#define SMOLC_DARKER_MAGENTA	SMOL_RGB(63,    0, 255)
#define SMOLC_DARKEST_MAGENTA	SMOL_RGB(32,    0,  32)
...
//smol_canvas_width - Fetch canvas width
//Arguments:
// - smol_canvas_t* canvas -- The canvas that's width is to be acquired
int smol_canvas_width(smol_canvas_t* canvas);


//smol_canvas_height - Fetch canvas height
//Arguments:
// - smol_canvas_t* canvas -- The canvas that's height is to be acquired
int smol_canvas_height(smol_canvas_t* canvas);
...
int smol_canvas_width(smol_canvas_t* canvas) {
	return canvas->draw_surface.width;
}

int smol_canvas_height(smol_canvas_t* canvas) {
	return canvas->draw_surface.height;
}
...
smol_font_t* smol_load_default_font() {
...

	{
		...
		for(int i = 0; i < PXF_NUM_PRINTABLE_GLYPHS; i++) {
			smol__builtin_geom_data[data_offset + i * 2 + 0] = bitstream[i * 2 + 0];
...
smol_pixel_t smol_canvas_get_color(smol_canvas_t* canvas) {
	return smol_stack_back(canvas->color_stack, smol_pixel_t);
}

void smol_canvas_get_hsva(smol_canvas_t* canvas, smol_u16* hue, smol_u8* sat, smol_u8* val, smol_u8* alp) {
	smol_pixel_t c = smol_stack_back(canvas->color_stack, smol_pixel_t);

	smol_i16 bmax = c.r > c.g ? c.r : c.g > c.b ? c.g : c.b;
	smol_i16 bmin = c.r < c.g ? c.r : c.g < c.b ? c.g : c.b;
	smol_i16 bdiff = bmax - bmin;

	if(bmax == c.r) {
		*hue = (120 * (c.g - c.b) / c.r);
	}
	else if(bmax == c.g) {
		*hue = 120 + (120 * (c.b - c.r) / bdiff);
	} else if(bmax == c.b) {
		*hue = 240 + (120 * (c.g - c.r) / bdiff);
	}
...
void smol_canvas_shift_color(smol_canvas_t* canvas, smol_i16 hue, smol_i16 sat, smol_i16 val, smol_i16 alp) {

	smol_u16 h;
	smol_u8 s, v, a;
	smol_canvas_get_hsva(canvas, &h, &s, &v, &a);
	h += hue;
	s += sat;
	v += val;
	a += alp;	
	smol_canvas_set_color_hsva(canvas, h, s, v, a);
...
void smol_canvas_draw_arrow(smol_canvas_t* canvas, int x0, int y0, int x1, int y1, int tip_size) {
	...
	if(fabsf(nx + ny) >= 1.f) {
		...
		smol_canvas_draw_line(canvas, x1 - nx * tip_size + ny * tip_size, y1 - ny * tip_size - nx * tip_size,  x1, y1);
		smol_canvas_draw_line(canvas, x1 - nx * tip_size - ny * tip_size, y1 - ny * tip_size + nx * tip_size,  x1, y1);
	}
}
...
void smol_canvas_fill_circle(smol_canvas_t* canvas, int xc, int yc, int rad) {
	...
	do {
		...
		if((yc - y) >= 0 && (yc - y+1) < (int)canvas->draw_surface.height) {
			...
			for(int i = l; i < r; i++) {
				smol_image_blend_pixel(&canvas->draw_surface, i, yc - y+1, color, blend);
			}
		}
...
void smol_canvas_draw_rect(smol_canvas_t* canvas, int x, int y, int w, int h) {
	...
	int l = (x < left) ? left : x;
	int t = (y < top) ? top : y;
	int r = ((l + w) > right) ? right : l+w;
	int b = ((t + h) > bottom) ? bottom : t+h;

	if(y >= top && y < bottom) {
		for(int i = l; i < r; i++) {
			smol_image_blend_pixel(&canvas->draw_surface, i, y, color, blend);
		}
	}
	if((y + h - 1) >= top && (y + h - 1) < bottom) {
		for(int i = l; i < r; i++) {
			smol_image_blend_pixel(&canvas->draw_surface, i, y + h - 1, color, blend);
		}
	}
	if(x >= left && x < right) {
		for(int i = t + 1; i < b - 1; i++) {
			smol_image_blend_pixel(&canvas->draw_surface, x, i, color, blend);
		}
	}

	if((x + w - 1) >= left && (x + w - 1) < right) {
		for(int i = t + 1; i < b - 1; i++) {
			smol_image_blend_pixel(&canvas->draw_surface, x + w - 1, i, color, blend);
		}
	}
}
